...
func (t *SimpleChaincode) createOrUpdateAsset(stub shim.ChaincodeStubInterface, args []string) ([]byte, error) {
	...
	assetID = *stateIn.AssetID
	container_health := strings.TrimSpace(*stateIn.Health)
	...
	if err != nil || len(assetBytes) == 0 {
		...
		if container_health=="bad" || container_health=="Bad" || container_health=="BAD"
		{
			*stateIn.Overallstatus = "compromised" 
		}
		else
		{
			*stateIn.Overallstatus = "good"
		}
